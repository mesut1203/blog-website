...
import { Leaf, Calendar, Tag, Search, SlidersHorizontal, ArrowRight, ChevronDown } from 'lucide-react';
import { Link } from 'react-router-dom';
import { getBlogsWithCategories } from '../lib/api';
...
export default function Blog() {
    ...
    useEffect(() => {
        async function loadBlogs() {
            try {
                const data = await getBlogsWithCategories();
                setBlogs(data);
            ...
            <div className="max-w-7xl mx-auto px-6 md:px-12 py-12 lg:py-16 -mt-12 relative z-30">
                ...
                <div className="mb-12 bg-white/80 backdrop-blur-xl p-4 rounded-2xl shadow-xl shadow-emerald-900/10 flex flex-col md:flex-row gap-4 items-center justify-between border border-emerald-100 relative z-40">
                    ...
                    <div className="relative w-full md:w-64">
                        ...
                        {isFilterOpen && (
                            <div className="absolute top-full mt-2 w-full bg-white rounded-2xl shadow-xl shadow-emerald-900/10 border border-emerald-100 overflow-hidden z-30">
                                <div className="max-h-60 overflow-y-auto">
                                    {categories.map(category => (
                                        <button
                                            ...
                                            className={`w-full text-left px-5 py-3 text-sm font-semibold transition-colors flex items-center justify-between ${selectedCategory === category
                ...
                <div className="mb-10 flex items-center justify-between text-emerald-800/70 font-semibold px-2 relative z-20">
                    <p>Showing {filteredBlogs.length} {filteredBlogs.length === 1 ? 'result' : 'results'}</p>
                    {searchQuery && (
                        <p>for "{searchQuery}"</p>
                    )}
                </div>
