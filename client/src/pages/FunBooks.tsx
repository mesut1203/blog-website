    ...
    <section className="relative w-full overflow-hidden py-24 md:py-32 lg:py-40">
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 text-center text-white">
            <h1 className="text-4xl md:text-6xl lg:text-7xl font-extrabold leading-tight tracking-tight mb-6">
                Adventures in <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-200 to-indigo-300">Literature.</span>
            </h1>
            <p className="text-lg md:text-2xl text-blue-50/80 max-w-3xl mx-auto font-medium leading-relaxed">
                Exploring worlds beyond ours through the pages of great books, hobbies, and the joy of reading.
            </p>
        </div>
    </section>
);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const targetCategory = categories.find(c => c.name.toLowerCase() === 'fun & books' || c.name.toLowerCase() === 'books');
                ...
                <section className="w-full max-w-7xl mx-auto px-6 py-20">
                    {loading ? (
                        ...
                        <div className="text-center py-32 bg-white rounded-3xl border border-emerald-100 mx-6 shadow-sm">
                            <h3 className="text-2xl font-bold text-emerald-900 mb-2">No chapters opened yet.</h3>
                            <p className="text-emerald-600">The library is quiet today. Return soon for new stories.</p>
                        </div>
                    ) : (
                        <>
                            {featuredPost && <FeaturedPost post={featuredPost} categoryName="Fun & Books" />}

                            {remainingPosts.length > 0 && (
                                <div>
                                    <h2 className="text-xl font-bold text-emerald-950 mb-8 border-b-2 border-emerald-100 pb-2 inline-block">Latest Readings</h2>
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                                        {remainingPosts.map(post => (
                                            <PostCard key={post.id} post={post} categoryName="Fun & Books" />
                                        ))}
                                    </div>
                                </div>
                            )}
