...
import { ArrowRight, Calendar, Tag, User, Target, TrendingUp, Sparkles, Search, Compass, SlidersHorizontal, ChevronDown } from 'lucide-react';
import { Link } from 'react-router-dom';

const SelfHero = () => (
    <section className="relative w-full overflow-hidden py-24 md:py-32 lg:py-40">
        <div className="absolute inset-0 z-0 bg-[url('https://plus.unsplash.com/premium_photo-1661963810374-85a874671aa8?w=900&auto=format&fit=crop')] bg-cover bg-center"></div>
        <div className="absolute inset-0 z-0 bg-black/40"></div>
        <div className="relative z-10 max-w-5xl mx-auto px-6 text-center text-white">
            <span className="text-emerald-400 font-bold tracking-[0.2em] uppercase text-sm mb-6 block">Self</span>
            <h1 className="text-4xl md:text-6xl lg:text-7xl font-extrabold leading-tight tracking-tight mb-6">
                The Journey of <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-200 to-teal-300">Self.</span>
            </h1>
            <p className="text-lg md:text-2xl text-emerald-50/80 max-w-3xl mx-auto font-medium leading-relaxed">
                Reflections on personal growth, identity, and the continuous evolution of the individual.
            </p>
        </div>
    </section>
);
...
export default function Self() {
    ...
    const [activeFilterId, setActiveFilterId] = usePersistedState('self_filter', 'All');
    const [searchQuery, setSearchQuery] = usePersistedState('self_search', '');
    const [debouncedSearch, setDebouncedSearch] = useState(searchQuery);
    const [isFilterOpen, setIsFilterOpen] = useState(false);
    const [currentPage, setCurrentPage] = usePersistedState('self_page', 1);
    const limit = 6;
    ...
    useEffect(() => {
        async function load() {
            try {
                ...
                setCategories(cats);
                const root = cats.find(c => c.name.toLowerCase() === 'self' && !c.parent_id);
                if (root) {
    ...
    const subCategories = useMemo(() => {
        const root = categories.find(c => c.name.toLowerCase() === 'self' && !c.parent_id);
    ...
    const getIcon = (name: string) => {
        ...
        if (n.includes('growth') || n.includes('evolve')) return <TrendingUp className="w-4 h-4" />;
        if (n.includes('identit') || n.includes('user') || n.includes('myself')) return <User className="w-4 h-4" />;
        if (n.includes('mind') || n.includes('mental')) return <Compass className="w-4 h-4" />;
        if (n.includes('soul') || n.includes('spirit')) return <Sparkles className="w-4 h-4" />;
        return <Target className="w-4 h-4" />;
    };

    const selectedLabel = activeFilterId === 'All' ? 'All Reflections' : (subCategories.find(c => c.id === activeFilterId)?.name ?? 'Filter');
            ...
            <div className="max-w-7xl mx-auto px-6 md:px-12 py-12 lg:py-16 relative z-30">
                <div className="mb-12 bg-white/80 backdrop-blur-xl p-4 rounded-2xl shadow-xl shadow-emerald-900/10 flex flex-col md:flex-row gap-4 items-center border border-emerald-100 relative z-40">
                    <div className="relative w-full md:flex-1 group">
                        ...
                        <input type="text" placeholder="Search through reflections and insights..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full pl-12 pr-4 py-3.5 bg-emerald-50/50 border border-emerald-100/80 rounded-2xl focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 text-emerald-950 placeholder-emerald-600/50 transition-all font-medium shadow-sm" />
                    </div>
                        ...
                        {isFilterOpen && (
                            <div className="absolute top-full mt-2 w-full bg-white rounded-2xl shadow-xl border border-emerald-100 overflow-hidden z-50 py-2">
                                <button onClick={() => { setActiveFilterId('All'); setIsFilterOpen(false); }} className={`w-full text-left px-5 py-3 text-sm font-semibold flex items-center justify-between transition-colors ${activeFilterId === 'All' ? 'bg-emerald-50 text-emerald-700' : 'text-emerald-900 hover:bg-emerald-50/50'}`}>
                                    <span className="flex items-center gap-2"><Sparkles className="w-4 h-4 text-emerald-500" />All Reflections</span>
                ...
                {loading && allPosts.length === 0 ? <div className="flex justify-center items-center h-64"><div className="w-12 h-12 border-4 border-emerald-200 border-t-emerald-600 rounded-full animate-spin"></div></div> : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        ...
                        {paginatedPosts.length === 0 && (
                            <div className="col-span-full text-center py-32 bg-white/40 rounded-[3rem] border border-white/50">
                                <div className="bg-emerald-100/50 w-24 h-24 rounded-full flex items-center justify-center mx-auto mb-6"><Compass className="w-10 h-10 text-emerald-400" /></div>
                                <h3 className="text-3xl font-bold text-emerald-950 mb-3">A moment of silence.</h3>
                                <p className="text-emerald-700/70 text-lg max-w-sm mx-auto">No reflections found. Try a different category.</p>
